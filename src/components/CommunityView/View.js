...
class View extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      communityTitle: this.props.location !== undefined && this.props.location.state !== undefined ? this.props.location.state.communityTitle : this.props.demoCommunityTitle !== undefined ? this.props.demoCommunityTitle : null,
      layout: 'preset',
      addView: '',
      showModal: false,
      showView: false,
  ...
  componentDidMount() {
    ...
    console.log('view loaded', this.state.layout)
  }
  ...
  render() {
    let viewToRender = this.state.currentView === 'Enhanced' ? <GraphView setNode={v => this.setState({ nodes: v })} layout={this.state.layout} viewId={this.props.viewId} viewLinks={this.props.viewLinks} readLinks={this.props.readLinks} onViewClick={this.onViewClick} onNoteClick={noteId => this.props.openContribution(noteId, 'write')} updateParentLayoutProp={this.updateLayout} /> : <LightView />
    const authors = this.state.nodes.map(node => node?.data?.author)
    const result = authors.reduce((acc, curr) => ((acc[curr] = (acc[curr] || 0) + 1), acc), {})

    return (
      <div className='container-fluid d-flex flex-column' id='container-fluid-for-view-js'>
        <DialogHandler />
        <AttachPanel attachPanel={this.state.showAttachPanel} viewId={this.props.viewId} onClose={() => this.setState({ showAttachPanel: false })} />
        <NewRiseAboveModal show={this.state.showRiseAboveModal} handleClose={() => this.setState({ showRiseAboveModal: false })} />
        <div className='row'>{<TopNavBar currentView={this.state.currentView} onViewClick={this.onViewClick} goToDashboard={this.goToDashboard} communityTitle={this.state.communityTitle}></TopNavBar>}</div>
            ...
            <div className='row sidebar-list'>
              {this.props.isDemo === false ? (
                <div className='sidebar-list-col col col-sm col-md-12'>
                  <OverlayTrigger placement='top' delay={{ show: 250, hide: 400 }} overlay={this.renderTooltip({ message: 'Create New Contribution' })}>
                    <DropdownButton drop='right' className='dropdown-btn-parent' title={<i className='fas fa-plus-circle'></i>}>
                      <Dropdown.Item onClick={() => this.props.newNote(this.props.view, this.props.communityId, this.props.author._id)}>New Note</Dropdown.Item>

                      <Dropdown.Item onClick={() => this.newView()}>New View</Dropdown.Item>

                      <Dropdown.Item onClick={() => this.setState({ showAttachPanel: true })}>New Attachment</Dropdown.Item>

                      <Dropdown.Item onClick={() => this.props.newDrawing(this.props.viewId, this.props.communityId, this.props.author._id)}>New Drawing</Dropdown.Item>
                    </DropdownButton>
                  </OverlayTrigger>
                </div>
              ) : null}
              ...
              <div className='sidebar-list-col col col-sm col-md-12'>
                <ViewSettingsPopover currentView={this.state.currentView} switchView={this.switchView} />
              </div>
                  ...
                  <OverlayTrigger
                    ...
                    overlay={
                      <Popover id='layoutPopover'>
                        ...
                        <Popover.Content>
                          <Row>
                            <Button value='preset' onClick={this.updateLayout} className={this.state.layout === 'preset' ? 'activeLayout' : ''}>
                              Original
                            </Button>
                          </Row>
                          <Row>
                            <Button value='grid' onClick={this.updateLayout} className={this.state.layout === 'grid' ? 'activeLayout' : ''}>
                              Grid
                            </Button>
                          </Row>
                          <Row>
                            <Button value='circle' onClick={this.updateLayout} className={this.state.layout === 'circle' ? 'activeLayout' : ''}>
                              Circle
                            </Button>
                          </Row>
                          <Row>
                            <Button value='spread' onClick={this.updateLayout} className={this.state.layout === 'spread' ? 'activeLayout' : ''}>
                              Spread
                            </Button>
                          </Row>
                          <Row>
                            <Button value='cose' onClick={this.updateLayout} className={this.state.layout === 'cose' ? 'activeLayout' : ''}>
                              Cose
                            </Button>
                          </Row>
                        </Popover.Content>
                    ...
                    <Col>
                      <Row>
                        <Form onSubmit={this.handleNewViewSubmit} className='form'>
                          <Col>
                            <FormGroup>
                              <Label htmlFor='addView' style={{ fontSize: '1rem' }}>
                                Add View
                              </Label>
                ...
                {this.props.myViews.map((obj, i) => {
                  return (
                    <Row key={i} value={obj.title} className='mrg-05-top'>
                      <Col>
                        <Link onClick={() => this.onViewClick(obj._id)}> {obj.title} </Link>
                      </Col>
                    </Row>
                  )
                })}
